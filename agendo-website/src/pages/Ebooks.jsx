...
export default function Ebooks() {
  ...
  const handleComprarAgora = (ebook) => {
    ...
    setTimeout(() => {
      alert('Sistema de pagamento em breve! Envie email para contato@agendoas.com');
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-6 py-12 md:py-16">
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">E-books AGENDO</h1>
          <p className="text-xl text-slate-600 max-w-2xl">Conteúdo profissional sobre gestão de OSCs e emendas parlamentares. Acesso imediato após o pagamento.</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
            ...
            <div key={ebook.id} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 flex flex-col">
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                <div className="flex gap-4 text-sm text-slate-600 mb-6 pb-6 border-b border-slate-200">
                  <div><span className="font-semibold">{ebook.pages}</span> páginas</div>
                  <div><span className="font-semibold">PDF</span> digital</div>
                </div>

                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm text-slate-600 mb-1">Preço</p>
                    <p className="text-4xl font-bold text-slate-900">R$ {ebook.price.toFixed(2)}</p>
                  </div>
                  <button onClick={() => handleComprarAgora(ebook)} disabled={loadingPayment && selectedEbook?.id === ebook.id} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white font-bold py-3 px-6 rounded-lg transition-all flex items-center justify-center gap-2">
                    {loadingPayment && selectedEbook?.id === ebook.id ? (
                      <span>⏳ Processando...</span>
                    ) : (
                      <span><Download size={20} /> Comprar Agora</span>
                    )}
                  </button>
                </div>
              </div>
        ...
        <div className="bg-blue-50 border border-slate-200 rounded-lg p-8 text-center">
          <h3 className="text-xl font-bold text-slate-900 mb-4">Compra 100% Segura</h3>
          <p className="text-slate-600">Seus dados serão protegidos pelo Mercado Pago. Acesso imediato ao PDF após confirmação do pagamento. Garantia de satisfação ou devolução em 7 dias.</p>
        </div>

        <div className="mt-16 max-w-2xl mx-auto">
          <h2 className="text-2xl font-bold text-slate-900 mb-8 text-center">Dúvidas Frequentes</h2>
          <div className="space-y-4">
            <details className="bg-white p-4 rounded-lg border border-slate-200 cursor-pointer">
              <summary className="font-semibold text-slate-900">Como recebo meu ebook?</summary>
              <p className="text-slate-600 mt-4">Você receberá um email com o link de download do PDF imediatamente após o pagamento.</p>
            </details>
            <details className="bg-white p-4 rounded-lg border border-slate-200 cursor-pointer">
              <summary className="font-semibold text-slate-900">Posso compartilhar o ebook?</summary>
              <p className="text-slate-600 mt-4">O ebook é para uso pessoal. Grupos e equipes podem adquirir múltiplas cópias com desconto.</p>
            </details>
            <details className="bg-white p-4 rounded-lg border border-slate-200 cursor-pointer">
              <summary className="font-semibold text-slate-900">Existe garantia de reembolso?</summary>
              <p className="text-slate-600 mt-4">Sim! Reembolso total em até 7 dias após a compra, sem perguntas.</p>
            </details>
            <details className="bg-white p-4 rounded-lg border border-slate-200 cursor-pointer">
              <summary className="font-semibold text-slate-900">O conteúdo é atualizado?</summary>
              <p className="text-slate-600 mt-4">Sim! Atualizações anuais. Compradores anteriores recebem notificação.</p>
            </details>
          </div>
        </div>
      </div>
      ...
      <div className="bg-blue-600 text-white py-12 mt-16">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="text-blue-100 mb-8">Nossos especialistas estão prontos para orientar sua organização.</p>
          <a href="/contato" className="inline-block bg-white text-blue-600 font-bold py-3 px-8 rounded-lg hover:bg-blue-50">
